...
final class ProductionValuesBoundaryTest extends TestCase
{
    public function testMoneyPreservesExactDigitsAtInclusiveBounds(): void
    {
        ...
        foreach (['1.2345678', '1000000000000000000', '1e3', '+1', '01', "1\n", '1 ', ''] as $invalid) {
            $this->assertThrows(static fn () => ProductionValues::parseMoneyValue(
                (object) ['amount' => $invalid, 'currency' => 'USD']), RenderException::class,
                'Noncanonical money is refused by the public parser.');
        }
        foreach (['usd', 'US', 'USDD', "USD\n", ' USD'] as $invalid) {
            $this->assertThrows(static fn () => ProductionValues::parseMoneyValue(
                (object) ['amount' => '1', 'currency' => $invalid]), RenderException::class,
    ...
    public function testChartAndTableCountLimitsRefuseBeforeItemInspection(): void
    {
        ...
        $this->assertThrows(static fn () => ProductionValues::parseChartSpec($chart), RenderException::class,
            'Two hundred and one labels refuse without inspecting the excess item.');
        foreach ([NAN, INF, -INF, 1.0e16, '1'] as $number) {
            $this->assertThrows(static fn () => ProductionValues::parseChartSpec((object) ['type' => 'bar',
                'labels' => ['A'], 'datasets' => [(object) ['label' => 'Series', 'values' => [$number]]]]),
                RenderException::class, 'Approximate chart numbers must remain finite and bounded.');
        }
        ...
        $table->rows[] = [];
        $this->assertThrows(static fn () => ProductionValues::parseTableDocument($table), RenderException::class,
    ...
    public function testDrawingTokensAndCoordinatesMatchTheirWholeBoundedInput(): void
    {
        ...
        $this->assertSame(4096, ProductionValues::parseDrawingDocument($drawing)->height, 'Dimension bound is inclusive.');
        foreach (["#abcdef\n", "brand/primary\n", 'url(javascript:alert(1))'] as $color) {
            $candidate = clone $drawing;
            $candidate->strokes = [(object) ['color' => $color, 'width' => 1,
                'points' => [(object) ['x' => 1, 'y' => 1]]]];
            $this->assertThrows(static fn () => ProductionValues::parseDrawingDocument($candidate),
                RenderException::class, 'A color token must match every input byte.');
        }
        foreach ([-1, 4097, NAN, INF] as $coordinate) {
            ...
            $this->assertThrows(static fn () => ProductionValues::parseDrawingDocument($candidate),
                RenderException::class, 'Coordinates stay inside the declared drawing.');
        }
    }
}
